...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f + 5, label_size.y + style.FramePadding.y * 2.0f);
    ...
    static std::map<ImGuiID, int> color_anim;
    auto it_color = color_anim.find(id);

    // Render
    const ImU32 col = ImColor(it_color->second, it_color->second, it_color->second, 165);

    if (it_color == color_anim.end())
    {
        color_anim.insert({ id, 13 });
        it_color = color_anim.find(id);
    }

    if (hovered)
    {
        if (it_color->second < 33)
            it_color->second += 3;
    }
    else
    {
        if (it_color->second > 13)
            it_color->second -= 3;
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f) + 155, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    ImColor _active = ImColor(13, 13, 13, 165);
    bool hovered, held;
    ...
    static std::map<ImGuiID, int> color_anim_r;
    auto it_color_r = color_anim_r.find(id);

    static std::map<ImGuiID, int> color_anim_g;
    auto it_color_g = color_anim_g.find(id);

    static std::map<ImGuiID, int> color_anim_b;
    auto it_color_b = color_anim_b.find(id);

    if (it_alpha == alpha_anim.end())
    {
        alpha_anim.insert({ id, 55 });
        it_alpha = alpha_anim.find(id);
    }
    if (it_color_r == color_anim_r.end())
    {
        color_anim_r.insert({ id, 13 });
        it_color_r = color_anim_r.find(id);
    }
    if (it_color_g == color_anim_g.end())
    {
        color_anim_g.insert({ id, 13 });
        it_color_g = color_anim_g.find(id);
    }
    if (it_color_b == color_anim_b.end())
    {
        color_anim_b.insert({ id, 13 });
        it_color_b = color_anim_b.find(id);
    }
    ...
    if (*v)
    {
        ...
        if (it_color_r->second < 108)
            it_color_r->second += 5;

        if (it_color_g->second < 109)
            it_color_g->second += 5;

        if (it_color_b->second < 163)
            it_color_b->second += 5;
    }
    else
    {
        if (it_alpha->second > 55)
            it_alpha->second -= 5;

        if (it_color_r->second > 13)
            it_color_r->second -= 5;

        if (it_color_g->second > 13)
            it_color_g->second -= 5;

        if (it_color_b->second > 13)
            it_color_b->second -= 5;
    }
    _active = ImColor(it_color_r->second, it_color_g->second, it_color_b->second, it_alpha->second);

    //_active = ImColor(108, 109, 163, it_alpha->second);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min + ImVec2(201, 1), check_bb.Max + ImVec2(199, -1), _active, true, style.FrameRounding + 5);
    ...
    else if (*v)
    {
        ...
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad) + ImVec2(200, 0), check_col, square_sz - pad * 2.0f);
    }

    ImVec2 label_pos = ImVec2(check_bb.Min.x + 10, check_bb.Min.y + style.FramePadding.y);
    if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(90, 4), window->DC.CursorPos + ImVec2(w + 90, label_size.y + style.FramePadding.y * 2.0f + 4));
    ...
    const ImU32 frame_col = ImColor(it_alpha->second, it_alpha->second, it_alpha->second, 133);
    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    RenderNavHighlight(bb, id);

    if (it_alpha == color_alpha.end())
    {
        color_alpha.insert({ id, 13 });
        it_alpha = color_alpha.find(id);
    }

    if (hovered) {
        if (it_alpha->second < 35)
            it_alpha->second += 1;
    }
    else {
        if (it_alpha->second > 13)
            it_alpha->second -= 1;
    }


    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), frame_col, style.FrameRounding + 5, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersAll);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(bb.Min + style.FramePadding, ImVec2(value_x2, bb.Max.y - 1), preview_value, NULL, NULL);
    }
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Min.x - 80, bb.Min.y + style.FramePadding.y), label);
...
#include "../menu/examples/example_win32_directx11/menu.h"
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10,20), window->DC.CursorPos + ImVec2(w + 70, label_size.y + style.FramePadding.y * 2.0f + 10));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    // Draw frame
    const ImU32 frame_col = ImColor(13, 13, 13, 165);
    RenderNavHighlight(frame_bb, id);

    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, g.Style.FrameRounding + 5);
    ...
    if ((frame_bb.Min.x + it_filing->second.value) < grab_bb.Min.x) {
        it_filing->second.speed = (grab_bb.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 8.f;
        it_filing->second.value += it_filing->second.speed;
    }
    else if ((frame_bb.Min.x + it_filing->second.value) > grab_bb.Min.x) {
        it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - grab_bb.Min.x)) / 8.f;
        it_filing->second.value -= it_filing->second.speed;
    }
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y), ImVec2(frame_bb.Min.x + it_filing->second.value + 8, total_bb.Max.y), ImColor(78, 79, 133, 255), 5);
    //window->DrawList->AddRectFilledMultiColor(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y), ImVec2(frame_bb.Min.x + it_filing->second.value + 8, total_bb.Max.y), ImColor(108, 109, 163, 255), ImColor(78, 79, 93, 255), ImColor(48, 49, 63, 255), ImColor(48, 49, 63, 255));
    //RenderFrame(frame_bb.Min, grab_bb.Max, ImColor(108, 109, 163, 255), true, g.Style.FrameRounding + 5);

    static std::map<ImGuiID, float> nameanim_animation0;
    auto it_nameanim0 = nameanim_animation0.find(id);
    if (it_nameanim0 == nameanim_animation0.end())
    {
        nameanim_animation0.insert({ id, 11.f });
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_filing->second.value + 5, grab_bb.Min.y + 3), it_nameanim0->second - 3, ImColor(108, 109, 163, 255), 10);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    //RenderTextClipped(frame_bb.Min, frame_bb.Max + ImVec2(160, 0), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

    ImGui::GetWindowDrawList()->AddText(menu.nameTable, 15, frame_bb.Max - ImVec2(18, 30), ImColor(78, 78, 78, 255), value_buf); //Value

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x - 4, frame_bb.Min.y + style.FramePadding.y - 22), label); //name
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        const ImU32 col = ImColor(33, 33, 33,133);
