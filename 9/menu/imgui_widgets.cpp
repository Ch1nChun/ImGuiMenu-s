...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v) {
        ...
        RenderCheckMark(window->DrawList, ImVec2(total_bb.Min.x + 1, (total_bb.Min.y + total_bb.Max.y) / 2 - 6.5), ImColor(29, 29, 29), 9.0f);
    }
    else {
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(15, 2), ImVec2(max(frame_bb.Min.x + 14, grab_bb.Max.x * 1 + 2), frame_bb.Max.y) + ImVec2(0, 2), ImColor(187, 150, 159), ImColor(187, 150, 159), ImColor(131, 107, 113), ImColor(131, 107, 113));
    ...
    window->DrawList->AddRectFilled(ImVec2(max(frame_bb.Min.x + 20 + 3, grab_bb.Max.x * 1 - 5 + 3), frame_bb.Min.y),ImVec2(max(frame_bb.Min.x + 14 + 3, grab_bb.Max.x * 1 - 1 + 3), frame_bb.Max.y + 2) + ImVec2(0, 2),
        ImColor(160, 160, 160, int(255 * fill * 1.5f)), style.FrameRounding);


    window->DrawList->AddRect(ImVec2(max(frame_bb.Min.x + 20 + 3, grab_bb.Max.x * 1 - 5 + 3), frame_bb.Min.y) - ImVec2(1, 1), ImVec2(max(frame_bb.Min.x + 14 + 3, grab_bb.Max.x * 1 - 1 + 3), frame_bb.Max.y + 2) + ImVec2(0, 2) + ImVec2(1, 1),
...

    {
        ...
        strcpy(temp_buf, text);  // Копируем "text" во временный буфер
        strcat(temp_buf, value_buf);  // Добавляем содержимое value_buf во временный буфер

        // Копируем объединенную строку обратно в value_buf
        ...
        RenderText(ImVec2(frame_bb.Max.x - label_buffer_size.x - 110, frame_bb.Min.y - g.Font->FontSize - 2), value_buf);
        PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        RenderFrame(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + 5 + ImVec2(1, 1), ImColor(29, 29, 29), true, 3);
        RenderFrame(frame_bb.Min, frame_bb.Max + 5, ImColor(34, 34, 34), true, 3);
    }
