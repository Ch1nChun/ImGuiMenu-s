...
evo::combo_t::combo_t( std::string label, int* value, std::vector<std::string> items ) {
	this->label = label;
	this->value = value;
	this->items = items;
}

void evo::combo_t::paint( ) { 
	...
	auto animation = animation_controller.get( this->label + "#animgovertes" + std::to_string( _container->get_id( ) ) + animation_controller.current_child );
	...
	auto animation2 = animation_controller.get( this->label + "#focusemanim" + std::to_string( _container->get_id( ) ) + animation_controller.current_child );
	...
	evo::_render->add_text( this->base_window.x + 10, this->base_window.y + 5, _container->window_text.modify_alpha( 155 * _container->anim_controler ).blend( _container->window_accent, animation.value ), evo::fonts_t::_default2,
							animation.value > 0 ? this->items[ *value ].c_str( ) : this->label.c_str( ) );
	...
	if ( animation2.value > 0.f ) {
		...
		for ( int i = 0; i < this->items.size( ); i++ ) {
			...
			animation_2.adjust( animation_2.value + 3.f * animation_controller.get_min_deltatime( 0.4f ) * ( *this->value == i && state_dropdown ? 1.f : -1.f ) );
...
void evo::combo_t::input( ) { 
	...
	if ( str_index_dropdown == _container->get_id( ) && _input->key_pressed( VK_LBUTTON )
		 && !_input->mouse_in_box( this->base_window + evo::vec2_t( 0, 32 ), { ( float )_container->group_width - 2, float( this->items.size( ) ) * 22 } ) ) {
	...
	if ( !_container->base_handler[ 0 ] && !theme::colorpicker_is_opened && _container->base_opened[ _container->base_handler_t::multicombo ] == -1 && time_dropdown == -1 &&
	...
	_container->base_opened[ _container->base_handler_t::combo ] = time_dropdown;
	...
	_container->base_opened_state[ _container->base_handler_t::combo ][ _container->get_id( ) ] = this->focused;
}
