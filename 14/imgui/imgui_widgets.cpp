...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    float t = (g.ActiveId == id) || (state && user_scroll_active) ? 1.0f : 0.0f;
    float ANIM_SPEED = 0.23f;
    if (g.LastActiveId == g.CurrentWindow->GetID(label)) {
        float t_anim = ImSaturate(g.LastActiveIdTimer / ANIM_SPEED);
        t = (g.ActiveId == id) || (state && user_scroll_active) ? (t_anim) : (1.0f - t_anim);
    }

    ImVec4 check_col_vec4 = ImGui::ColorConvertU32ToFloat4(ImGui::GetColorU32(ImGuiCol_CheckMark));

    ImU32  col_bg = ImGui::GetColorU32(ImLerp(ImGui::ColorConvertU32ToFloat4(ImGui::GetColorU32(ImGuiCol_FrameBgHovered)),
        ImVec4(check_col_vec4.x + 0.03f,
            check_col_vec4.y + 0.03f,
            check_col_vec4.z + 0.3f, t), (t / 2) + (t > 0 ? 0.1 : 0)));

    RenderFrame(ImVec2(frame_bb.Min.x - t*1.5, frame_bb.Min.y - t*1.5), ImVec2(frame_bb.Max.x + t*1.5, frame_bb.Max.y + t*1.5), col_bg, true, 4.f);
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min-ImVec2(5,5), frame_bb.Max + ImVec2(5, 5), ImColor(120,120,120,120), true, 6.f);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_ButtonHovered));
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x - 5 + style.ItemInnerSpacing.x, frame_bb.Min.y - 19 - style.FramePadding.y), label);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        //RenderFrame(bb_enlarged.Min, bb_enlarged.Max, col, false, 8.0f);
        GetWindowDrawList()->AddRectFilled(bb_enlarged.Min + ImVec2(8, 0), bb_enlarged.Max - ImVec2(8, 0), col, 4.0f);
