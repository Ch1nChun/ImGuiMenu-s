...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        static bool linestate = true;
        static float linethick = 1.0f;

        if ((flags & ImGuiWindowFlags_FalconTriangle)) //Bg triangle
            window->DrawList->AddTriangleFilled(ImVec2(window->Pos.x + 500, window->Pos.y), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 10), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 450), ImColor(26, 25, 30)),
            window->DrawList->AddLine(ImVec2(window->Pos.x + 500, window->Pos.y), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 450), ImColor(63, 140, 254, int(linethick * 127.5)), linethick);

        
        if (linestate)
        {
            linethick += (0.003f * (60 / GetIO().Framerate));
            if (linethick >= 2.f)
                linestate = false;
        }
        else if (!linestate) {
