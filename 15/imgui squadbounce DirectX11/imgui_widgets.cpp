...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else
    {
        ...
        ItemSize(ImRect(total_bb.Min, total_bb.Max - ImVec2(12, 0)));
    ...
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::element::combo::filling), 4.f);
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? c::element::text : c::element::text_active);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.GetBL().y), GetColorU32(c::element::text_active));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputTextEx("c", "hex", buf, IM_ARRAYSIZE(buf), ImVec2(190, 40), ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_NoSetKeyOwner | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups;
        ...
        float min_w = window->DC.MenuColumns.DeclColumns(icon_w, label_size.x, 0.0f, 25); // Feedback to next frame
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel + 20, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        pressed = Selectable("", menu_is_open, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 25));

        GetWindowDrawList()->AddText(font::icomoon, 14.f,text_pos - ImVec2(30, -4), GetColorU32(c::element::text_active), icon);

        GetWindowDrawList()->AddText(font::inter_medium_small, 14.f, text_pos - ImVec2(0, -4), GetColorU32(c::element::text_active), label);
        if (menu_is_open) {

            GetWindowDrawList()->AddText(font::icomoon, 14.f, pos + ImVec2(offsets->OffsetMark + extra_w + 6, 4), GetColorU32(c::element::text_active), "l");
