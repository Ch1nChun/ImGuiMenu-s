...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 50, pos.y +50));
    ...
    it_hover->second = ImClamp(it_hover->second + (1.8f * ImGui::GetIO().DeltaTime * (hovered || IsItemActive() ? 1.f : -1.f)), 0.0f, 0.3f);
    ...
    ImVec4 bg = ImLerp(ImVec4{ 36 / 255.f, 47 / 255.f, 65 / 255.f, 1.0f }, ImVec4{ 31 / 255.f, 64 / 255.f, 97 / 255.f, 1.f }, it_filled->second);
    ImVec4 text = ImLerp(ImVec4{ 114 / 255.f, 120 / 255.f, 155 / 255.f, 1.0f }, ImVec4{ 45 / 255.f, 143 / 255.f, 206 / 255.f, 1.f }, it_filled->second);

    ImGui::GetForegroundDrawList()->AddCircleFilled(ImVec2(bb.Min.x + 25, bb.Min.y + 25), 16, ImColor(bg), 6);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (25.45f - label_size.x / 2), bb.Min.y + (52 / 2 - label_size.y / 2)), ImColor(114 / 255.f, 120 / 255.f, 155 / 255.f, 0.9f), icon);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (25.45f - label_size.x / 2), bb.Min.y + (52 / 2 - label_size.y / 2)), ImColor(45 / 255.f, 143 / 255.f, 206 / 255.f, it_filled->second), icon);

    ImGui::PushFont(ImGui::GetIO().Font);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 25 - CalcTextSize(label).x / 2, bb.Min.y + (46 - CalcTextSize(label).y / 2)), ImColor(text), label);
    ImGui::PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImVec4 text = ImLerp(ImVec4{ 67 / 255.f, 72 / 255.f, 92 / 255.f, 1.0f }, ImVec4{ 45 / 255.f, 143 / 255.f, 206 / 255.f, 1.f }, it_filled->second);
   // RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(26, 29, 34, int(255 * GetStyle().Alpha)), false, 4);
    window->DrawList->AddCircle(check_bb.Min + ImVec2(pad * 2.75, pad * 2.75), 6, ImColor(text), 6);
    window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(pad * 2.75, pad * 2.75), *v  || it_filled->second > 0.12f ? 4 * it_filled->second : 0, ImColor(text), 6);
    if (label_size.x > 0.0f) {
        GetWindowDrawList()->AddText(text_bb.GetTL(), ImColor(67, 72, 92, int(155 * GetStyle().Alpha)), label);
        GetWindowDrawList()->AddText(text_bb.GetTL(), ImColor(45, 143, 206, int(255 * it_filled->second * GetStyle().Alpha)), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Max.y - 6), frame_bb.Max + ImVec2(-2, 1), ImColor(5, 5, 7, 160), 213128);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(26, 29, 34), 4);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
       ...
       // window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, ImColor(180, 180, 180), 2, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            if (!popup_open)
            {
                window->DrawList->AddLine(ImVec2(value_x2 + 4, frame_bb.Min.y + 6 + 2), ImVec2(value_x2 + 8, frame_bb.Min.y + 10 + 2), ImColor(51, 58, 64));
                window->DrawList->AddLine(ImVec2(value_x2 + 8, frame_bb.Min.y + 10 + 2), ImVec2(value_x2 + 12, frame_bb.Min.y + 5 + 2), ImColor(51, 58, 64));

                window->DrawList->AddLine(ImVec2(value_x2 + 3, frame_bb.Min.y + 6 + 2), ImVec2(value_x2 + 8, frame_bb.Min.y + 11 + 2), ImColor(18, 22, 26));
                window->DrawList->AddLine(ImVec2(value_x2 + 8, frame_bb.Min.y + 11 + 2), ImVec2(value_x2 + 13, frame_bb.Min.y + 5 + 2), ImColor(18, 22, 26));
            }
            else
            {
                window->DrawList->AddLine(ImVec2(value_x2 + 4, frame_bb.Min.y + 6 + 2), ImVec2(value_x2 + 8, frame_bb.Min.y + 10 + 2), ImColor(51, 58, 64));
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(22 / 255.f, 24 / 255.f, 29 / 255.f, 0.9f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 2 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 26), GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
