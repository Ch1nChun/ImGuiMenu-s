...
#include <windows.h>
#include <vector>
#include <examples/example_win32_directx9/imgui_tricks.hpp>
...
bool ImGui::tab(const char* icon, bool selected)
{
    ...
    const ImRect bb1(pos, ImVec2(pos.x + sizes.x, pos.y + sizes.y));
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(31, 31, 31, int(255 * GetStyle().Alpha)));
    GetWindowDrawList()->AddRectFilled(ImVec2(bb1.Min.x, bb1.Min.y), ImVec2(bb1.Max.x, bb1.Max.y ), ImColor(255, 255, 255, int(255 * it_filled->second)));

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 14, bb.Min.y + 9), ImColor(255, 255, 255, int(255 * GetStyle().Alpha)), icon);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, ImVec2(pos.x + 20, pos.y + 20));
    ...
    window->DrawList->AddText(InterMedium, 16, ImVec2(total_bb.Min.x + 25, total_bb.Min.y + 15 - ImGui::CalcTextSize(label).y / 2), ImColor(255, 255, 255), label);

    ImGui::RenderFrame(total_bb.Min, total_bb.Max, ImColor(24, 25, 27), true);
      
    if (*v)
       RenderCheckMark(window->DrawList, { total_bb.Max.x - square_sz / 2.2f, total_bb.Min.y + square_sz / 6.1f }, ImColor(255, 255, 255), 10.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 20));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(24, 25, 27));
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(24, 25, 27));
    window->DrawList->AddRectFilled(grab_bb.Min, ImVec2(grab_bb.Min.x + 15, grab_bb.Max.y), ImColor(90, 90, 90));
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(31, 31, 31, int(255 * alpha)));
    parent_window->DrawList->AddRect(pos, p + size_arg, ImColor(21, 21, 21, int(255 * alpha)), true, 2, 2);

    parent_window->DrawList->AddText(InterMedium, 16, pos + ImVec2(10, 8), ImColor(255, 255, 255, int(255 * alpha)), name);
